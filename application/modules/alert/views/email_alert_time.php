<script>
	$(window).load(function() {    
		$("#add_alert").click(function(){
			...
			var alert_time = "<div class='col-md-12'><div class='col-md-1'>Before </div><div class='col-md-2'><select id='days' name='days[]' class='form-control'>";
			<?php for($i=0;$i<=15;$i++){
				echo "alert_time += '<option value=\"$i\">$i</option>';\n";
			}?>
			alert_time += "</select> days </div><div class='col-md-2'><select id='hours' name='hours[]'  class='form-control'>";
			<?php for($i=0;$i<=23;$i++){
				echo "alert_time += '<option value=\"$i\">$i</option>';\n";
			} ?>
			alert_time += "</select> hour </div><div class='col-md-2'><select id='minutes' name='minutes[]'  class='form-control'>";
			<?php for($i=0;$i<=59;$i++){
				echo "alert_time += '<option value=\"$i\">$i</option>';\n";
			} ?>
			alert_time += "</select> minutes </div><div class='col-md-3'>of appoinment time </div>";
			alert_time += "<div class='col-md-2'><label></label><a href='#' id='delete_alert"+alert_count+"' class='btn btn-danger btn-sm square-btn-adjust'>Delete</a></div></div>";
		...
		<div class="col-md-12">
				<div class="panel panel-primary" >
					...
					if($alert_occur == "APPNT") { ?>
						...
						<?php echo form_open('alert/save_email_alert_time/'.$alert_name.'/'.$alert_occur); ?>    	
							<div class="form-group">
								<a href="#" id="add_alert" class="btn btn-primary"/>Add Alerts</a>
							...
							<div class="form-group">
								...
								<a class="btn btn-primary" href="<?=site_url('alert/settings');?>"  />Cancel</a>
							</div>
						<?php echo form_close(); ?>    	
					</div>
					<?php } 
