...
class Alert extends CI_Controller {
	...
	function index(){
		...
		if (in_array("alert", $active_modules)) {
			...
			foreach($alerts as $alert){
				...
				foreach($future_doses as $doses){
					foreach($time_for_reminder_array as $time => $dose_time){
						if ((strtotime($now) >= (strtotime($dose_time) - 900)) && (strtotime($now) <= (strtotime($dose_time) + 900))){
			...
			foreach($alerts as $alert){
				...
				if (strtotime($now) >= (strtotime($time_for_reminder) - 900) && strtotime($now) <= (strtotime($time_for_reminder) + 900)){
					$alert_name = str_replace('email_alert_','',$alert['alert_name']);
					$alert_name = str_replace('sms_alert_','',$alert_name);
					$alert_name = str_replace('_to_patient','',$alert_name);
					$alert_name = str_replace('_to_doctor','',$alert_name);
			...
			if($this->alert_model->is_alert_enabled($sms_alert_name_to_patient)){
				if($patient_id != NULL){
					...
					if($mobile_number != ''){
						...
						$message = $this->generate_message($template,$patient_id,$doctor_id,$appointment_id,$visit_id,$payment_id,$dose_time);
			...
			if($this->alert_model->is_alert_enabled($email_alert_name_to_patient)){
				...
				if($patient_id != NULL){
					...
					if($email_id != ''){
						...
						$subject = $this->generate_message($subject_template,$patient_id,$doctor_id,$appointment_id,$visit_id,$payment_id,$dose_time);
	...
	function generate_message($template,$patient_id,$doctor_id,$appointment_id,$visit_id,$payment_id,$dose_time){
		//Clinic Name
		...
		if($doctor_id != NULL){
			//Doctor Name
			$doctor = $this->admin_model->get_user_detail($doctor_id);
	...
	function send_email($alert_name,$email_id,$subject,$message){
		...
		$this->alert_model->email_log($alert_name,$email_id,$subject,$message,$response,$current_time);
