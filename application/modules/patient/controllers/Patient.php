...
class Patient extends CI_Controller {
	...
	public function getPatient(){
		...
		$page = isset($display['page']) ? $display['page'] : 1;
		$limit = isset($display['limit']) ? $display['limit'] : 10;
		$offset = ($page  - 1) * $limit;
		$to = $page * $limit;

		$data = $this->patient_model->getPatient($offset, $limit, $sort, "list");
		$count = $this->patient_model->getPatient($offset, $limit, $sort, "count");
		$pages = ceil($count / $limit);
		$current = (($offset - 1) / $limit) + 1;
		$currentpage = (int)round($current);
		...
		$out['next'] = ($currentpage < $pages) ? $currentpage + 1 : $currentpage;
		$out['previous'] = ($currentpage > 1) ? $currentpage - 1 : 1;
		$out['offset'] = $offset;
		$out['data'] = $data;
		$out['limit'] = $limit;
