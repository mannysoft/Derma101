    ...
    $(window).load(function(){
		...
		foreach ($patients as $patient) {
			if ($i > 0) {
				...
				echo '{value:"' . $patient['email'] . '",id:"' . $patient['patient_id'] . '",display:"' . $patient['display_id'] . '",patient:"' . $patient['first_name'] . " " . $patient['middle_name'] . " " . $patient['last_name'] . '",email:"' . $patient['email'] . '"}';
			$i++;
		}?>];
		...
		$('#start_time').change(function() {
			...
			$('#end_time').val(moment(starttime, '<?=$morris_time_format; ?>').add('<?=$time_interval*60; ?>', 'minutes').format('<?=$morris_time_format; ?>'));
		...
		<div class="col-md-12">
			...
			<div class="panel panel-primary">
				...
				<div class="panel-body">
					...
					<div class="col-md-12">
						<div class="form-group">
						<h4>Change Visit-Type options</h4>
					<?php if(isset($appointment)){ ?>
						...
						<?php if ($status != 'Waiting') { ?>
							<a class="btn btn-primary btn-warning" href="<?=base_url() . "index.php/appointment/change_status/" . $appointment_id . "/Waiting";?>"><?php echo $this->lang->line('waiting');?></a>
						<?php } ?>
						...
						<?php if ($status != 'Cancel') { ?>
							<a class="btn btn-danger" href="<?=base_url() . "index.php/appointment/change_status/" . $appointment_id . "/Cancel";?>" ><?php echo $this->lang->line('cancel')." ".$this->lang->line('appointment');?></a>
						<?php } ?>
					<?php } ?>
					<a class="btn" href="<?=base_url() . "index.php/appointment/index/";?>"><?=$this->lang->line('back_to_app');?></a>
						</div>
					</div>
					<?php echo form_close() ?>
				</div>
