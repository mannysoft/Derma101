...
class Patient_model extends CI_Model {
    ...
    public function DumpData(){
    	...
    	for($i = 0; $i < 10000; $i++){
    		$data = array();
    		$str = "abcdefguyiqowieoua,xdfgjteyqoewroclxasdrutyowpqlkgnmckdlf";
    		$shuffled = str_shuffle($str);

   //  		$data['first_name'] = $shuffled;
			// $data['middle_name'] = $shuffled;
			// $data['last_name'] = $shuffled;
			// $data['display_name'] = $shuffled;
			// $data['phone_number'] = $shuffled;
			// $data['email'] = $shuffled . "@yahoo.com";
			// $data['address_line_1'] = $shuffled;
			// $data['address_line_2'] = $shuffled;
			// $data['city'] = $shuffled;
			// $data['state'] = $shuffled;
			// $data['postal_code'] = $shuffled;
			// $data['country'] = $shuffled;

			// $data['contact_id'] = $i + 1;
			// $data['patient_since'] = date('Y-m-d');
			// $data['display_id'] = $shuffled;
   //  		$data['gender'] = "M";
   	...
   	public function getPatient($offset, $limit, $sort, $type = "list"){

  		$this->db->order_by("followup_date", "asc");
		$now = date('Y-m-d');
		$this->db->select("*");
   		$this->db->join("contacts", 'contacts.contact_id = patient.contact_id', 'left');

   		if($sort){
   			$this->db->or_like("display_name", $sort, "both");
   			$this->db->or_like("phone_number", $sort, "both");
   			$this->db->or_like("email", $sort, "both");
   			$this->db->or_like("reference_by", $sort, "both");
   		}

		if($type == "list"){
			// $this->db->limit($offset, $limit);
			$query = $this->db->get("patient", $limit, $offset);
			return $query->result_array();
		}else{
    ...
    function insert_patient($contact_id) {
        $data['contact_id'] = $contact_id;
        $data['patient_since'] = date("Y-m-d");
        $data['display_id'] = $this->display_id($contact_id);
   ...
   public function get_contact_id($patient_id) {
	...
	public function get_followups($till_date){
		...
		$this->db->where('followup_date >=' , $now);
		$query = $this->db->get();
